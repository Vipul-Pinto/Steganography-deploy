...
class SteganographyEngine:
    ...
    def decode(cls, image_stream, password):
        ...
        if msg_length > len(flat_img) // 8:
             raise ValueError("Detected message length is impossibly large. Wrong password or not encoded?")
        ...
        <div id="encode-panel" class="glass rounded-2xl p-8 shadow-2xl animate-fade-in">
            <form action="/encode" method="post" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="text" required rows="5" placeholder="Type your secret message here... supports Emojis ??, Kanji ??, and standard text." class="w-full bg-gray-800 border border-gray-600 rounded-lg p-4 text-white placeholder-gray-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 mono transition-all"></textarea>
                </div>
